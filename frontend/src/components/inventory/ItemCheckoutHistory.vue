      ...
      <table class="min-w-full divide-y divide-gray-700 text-sm">
        ...
        <tbody class="divide-y divide-gray-700">
          <tr v-for="loan in loans" :key="loan.id">
            ...
            <td class="px-5 py-4"><span v-if="!loan.checkedInAt" class="rounded-full bg-amber-900 px-2.5 py-1 text-xs font-medium text-amber-200">Currently checked out</span><span v-else class="text-gray-300">Returned{{ loan.returnPlace ? ` to ${placeLabel(loan.returnPlace)}` : '' }}</span><div v-if="loan.note" class="mt-2 max-w-xs whitespace-pre-wrap text-xs text-gray-400">{{ loan.note }}</div></td>
          </tr>
        </tbody>
      </table>
    </div>
